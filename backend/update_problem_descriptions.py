#!/usr/bin/env python3
"""
Update existing problem descriptions with enhanced, detailed versions
"""
...
def update_problem_descriptions():
    """Update existing problems with enhanced descriptions"""
...
""",
    ...
    try:
        for title, new_statement in enhanced_problems.items():
            problem = db.query(Problem).filter(Problem.title == title).first()

            if problem:
                problem.statement = new_statement
                db.commit()
                updated_count += 1
...
if __name__ == "__main__":
    print("🚀 Updating Problem Descriptions...")
    print("=" * 50)
    update_problem_descriptions()
